...
(function () {
    ...
    var assistantKnowledge = [
        ...
        { keys: ['mission', 'purpose', 'vision', 'why medsaarthi'], reply: 'medGlobal\'s vision is: One Platform. One Connected Ecosystem. Accessible Medicines for Every Citizen. Its purpose is to empower every citizen through Access, Trust, Technology, Transparency and Healthcare.' },
        { keys: ['problem', 'healthcare reality', 'silos', 'challenge'], reply: 'India has strong medicine production capacity, but discovery, prescribing and supply often operate in silos. medGlobal focuses on making medicines more discoverable, trusted and accessible.' },
        { keys: ['why generic', 'generics', 'affordable medicine', 'cheaper medicine', 'low cost medicine'], reply: 'medGlobal aims to make affordable generic medicines easier to discover and trust by connecting medicine information, price comparison, equivalence evidence and access points.' },
        ...
        { keys: ['privacy', 'data sovereignty', 'audit trail', 'governance', 'regulatory'], reply: 'medGlobal emphasizes privacy-by-design, data sovereignty, audit trails, permissioned access, secure data exchange and regulatory alignment.' },
        ...
        { keys: ['for government', 'government'], reply: 'medGlobal describes government value through policy collaboration, regulatory alignment, public health initiatives, aggregated analytics and procurement insights.' },
        ...
        { keys: ['can you prescribe', 'prescribe me', 'medical advice', 'what medicine should', 'diagnose', 'dosage', 'increase my medicine', 'stop my medicine'], reply: 'I can explain medGlobal\'s healthcare platform and its prescription-intelligence capabilities, but I can\'t provide personal medical advice or prescribe medicines. Please consult a qualified healthcare professional.' },
        { keys: ['thank', 'thanks'], reply: 'You\'re welcome. I can help you explore medGlobal\'s platform, products, medicine discovery and healthcare ecosystem.' },
        { keys: ['bye', 'goodbye'], reply: 'Goodbye. The medGlobal Assistant is here whenever you want to explore the platform.' }
    ];

    var assistantQuickActions = ['What is medGlobal?', 'How does medicine discovery work?', 'Explore the products', 'AI Prescription Intelligence', 'How does it help pharmacies?', 'How does it help government?'];
    var assistantFallback = 'I don\'t have that information in my current medGlobal knowledge base. I can help with medGlobal\'s platform, products, medicine discovery, AI prescription intelligence, healthcare ecosystem, stakeholders, or business model.';
    ...
    function assistantReply(message) {
        ...
        if (assistantMedical.test(message) && !/platform|prescription intelligence|medicine discovery/i.test(message)) return 'I can explain medGlobal\'s healthcare platform and its prescription-intelligence capabilities, but I can\'t provide personal medical advice or prescribe medicines. Please consult a qualified healthcare professional.';
        var best = null;
        var bestScore = 0;
    ...
    function initAssistant() {
        ...
        shell.innerHTML = '<button class="ms-chatbot-launcher" id="ms-chatbot-launcher" type="button" aria-label="Open medGlobal Assistant" aria-expanded="false"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 11.5a7.5 7.5 0 0 1-8 7.5 8.8 8.8 0 0 1-3.2-.6L4 20l1.5-3.3A7.3 7.3 0 0 1 4.5 12 7.5 7.5 0 1 1 20 11.5Z"/><path d="M9 12h.01M12 12h.01M15 12h.01"/></svg></button><section class="ms-chatbot-panel" id="ms-chatbot-panel" aria-label="medGlobal Assistant" aria-hidden="true"><header class="ms-chatbot-header"><div class="ms-chatbot-identity"><span class="ms-chatbot-status"></span><div><strong>medGlobal Assistant</strong><span>Your guide to affordable medicine access</span></div></div><div class="ms-chatbot-header-actions"><button type="button" class="ms-chatbot-icon-btn" id="ms-chatbot-clear" aria-label="Clear chat" title="Clear chat">↺</button><button type="button" class="ms-chatbot-icon-btn" id="ms-chatbot-close" aria-label="Close chat" title="Close chat">×</button></div></header><div class="ms-chatbot-messages" id="ms-chatbot-messages" role="log" aria-live="polite"></div><form class="ms-chatbot-form" id="ms-chatbot-form"><textarea id="ms-chatbot-input" rows="1" placeholder="Ask about medGlobal..." aria-label="Message medGlobal Assistant"></textarea><button type="submit" aria-label="Send message"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m4 4 16 8-16 8 3-8-3-8Zm3 8h8"/></svg></button></form></section>';
        ...
        function welcome() {
            messages.innerHTML = '';
            addMessage('Hello! I\'m the medGlobal Assistant.\n\nI can help you understand our platform, products, medicine discovery system, AI prescription intelligence, healthcare ecosystem, and how medGlobal connects patients, doctors, pharmacies, distributors and government.\n\nWhat would you like to know?', 'assistant');
